...
而这里参照物的意思就是元素(参照物)的 scrollY，这个具体会在 update 的时候介绍到。因为各个元素的坐标都会在页面滚动的时候变化，所以坐标变化的关键指标就是页面的 scrollY。也就是说没有设置参照物的时候，元素的坐标，transform 都会根据 window.scrollY 变化，不过如果设置了参照物，那么就是根据参照物相对于页面顶部的举例来变化(这里的距离页面顶部的距离指的是元素的 posY)。

我们上面讲过，设置了 lax.preset 的 anchor 都是 self，也就是说参照物都是自己。除了 lax.preset，我们也可以自己在元素上设置`data-lax-anchor`,比如`#id, .class`等等。

所以这段代码就是根据参照物计算出参照物距离页面顶部的距离并赋值给`o["data-lax-anchor-top"]`。
...
4. 如果属性名是`data-lax-anchor`，说明参照物要改成`data-lax-anchor`的值，获取参照物相对于页面顶部的距离并将值赋给`o["data-lax-anchor-top"]`。
...
`var r = o["data-lax-anchor-top"] ? o["data-lax-anchor-top"]-y : y`
就和我们之前讲到的一样，这里他会判断元素 o 上是否有`data-lax-anchor-top`属性，如果有，就会计算元素相对于页面顶部的距离(因为页面向下滚动了，那么这个参照物相对于原来的位置也就减少了那么多的滚动距离)，如果没有则直接去 window.scrollY。
